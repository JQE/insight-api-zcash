...
ChartController.prototype.chart = function(req, res, next) {
  ...
  var high = req.params.high || self.node.services.bitcoind.height;
  var limit = req.params.limit || BLOCK_LIMIT;
  ...
  if (chartCached) {
    ...
    self.node.services.bitcoind.getBlockHashesRange(high, limit, function(err, hashes) {
      if (err) {
