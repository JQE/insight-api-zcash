...
BlockController.prototype.checkBlockHashorHeight = function(req, res, next) {
	var self = this;
	var hash = req.params.blockHash;
	if ((isNaN(parseFloat(hash)) || !isFinite(parseFloat(hash))) && 
       (hash.length < 64 || !isHexadecimal(hash))) {
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    ...
    self.node.getBlock(hash, function(err, block) {
      ...
      self.node.services.bitcoind.getBlockHeader(block.hash, function(err, info) {
        if (err) {
          return self.common.handleErrors(err, res);
        }
        var blockResult = self.transformBlock(block, info);
        if (blockResult.confirmations >= self.blockCacheConfirmations) {
          self.blockCache.set(block.hash, blockResult);
        }
...
BlockController.prototype.list = function(req, res) {
	var self = this;
	var high = req.query.high || self.node.services.bitcoind.height;
	var limit = req.query.limit || BLOCK_LIMIT
	var more = false;
	var next = high-limit-1;
	var prev = high+1;
	var moreTimestamp = high-limit;
	var isToday = (high == self.node.services.bitcoind.height);
  
	self.node.services.bitcoind.getBlockHashesRange(high,limit, function(err, blocks) {
	  if((err && err.code === -5) || (err && err.code === -8)) {
		  console.log(err);
		return self.common.handleErrors(null, res);
	  } else if(err) {
